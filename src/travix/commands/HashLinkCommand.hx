  ...
  public function install() {
    ...
    if(Os.isLinux && haxeVersion == '4.0.0-preview.4') {
      foldOutput('hashlink-install', function() {
        ...
        exec('wget', ['$HL_GITHUB_RELEASES_BASE_URL/$hlVersion/hl-$hlVersion.0-linux.tgz']);
        exec('tar', ['xvzf', 'hl-$hlVersion.0-linux.tgz']);

        // workaround for "error while loading shared libraries: libhl.so cannot open shared object file: No such file or directory"
        Sys.putEnv("LD_LIBRARY_PATH", 'hl-$hlVersion.0-linux'.fullPath());

        hlCommand = 'hl-$hlVersion.0-linux/hl';
        exec('chmod', ['u+x', hlCommand]);
      });
      return;
    }
    ...
    if(Os.isWindows) {
      foldOutput('hashlink-install', function() {
        ...
        run('curl', [ // https://stackoverflow.com/a/50200838/5116073
          "-sSLo", '$temp\\hashlink-$hlVersion.0-win.zip',
          '$HL_GITHUB_RELEASES_BASE_URL/$hlVersion/hashlink-$hlVersion.0-win.zip'
        ]);

        run('tar', [ // https://superuser.com/a/1473255/1139467
          "-C", temp,
          "-xvf", '$temp\\hashlink-$hlVersion.0-win.zip'
         ]);

        hlCommand = '$temp\\hashlink-$hlVersion.0-win\\hl.exe';
      });
      return;
    }
